...
class MockD1 {
  ...
  run(sql, args) {
    if (sql.startsWith("INSERT INTO checkins")) {
      ...
      if (this.checkins.some((item) => item.anonymous_id === anonymousId && item.mode === mode)) {
        throw new Error("UNIQUE constraint failed: checkins.anonymous_id, checkins.mode");
      }
      this.checkins.push({
        id,
    ...
    if (sql.startsWith("INSERT OR IGNORE INTO draw_results") && sql.includes("SELECT")) {
      const [id, anonymousId, checkinId, mode, drawnAt, prizeId, countMode] = args;
      if (this.drawResults.some((item) => item.anonymous_id === anonymousId && item.mode === mode)) {
    ...
    if (sql.startsWith("INSERT OR IGNORE INTO draw_results")) {
      const [id, anonymousId, checkinId, mode, drawnAt] = args;
      if (this.drawResults.some((item) => item.anonymous_id === anonymousId && item.mode === mode)) {
        return { meta: { changes: 0 } };
      }
      this.drawResults.push({
    ...
    if (sql.startsWith("UPDATE draw_results SET claimed_at")) {
      const [claimedAt, anonymousId, mode] = args;
      const result = this.drawResults.find(
        (item) => item.anonymous_id === anonymousId && item.mode === mode && item.result === "win"
      );
      if (!result) {
        return { meta: { changes: 0 } };
      }
  ...
  first(sql, args) {
    if (sql.includes("FROM checkins c LEFT JOIN draw_results")) {
      const [anonymousId, mode] = args;
      return this.rowForResult(anonymousId, mode) ?? null;
    }
  ...
  rowForResult(anonymousId, mode) {
    const checkin = this.checkins.find((item) => item.anonymous_id === anonymousId && item.mode === mode);
    if (!checkin) {
      return null;
    }
    const draw = this.drawResults.find((item) => item.anonymous_id === anonymousId && item.mode === mode);
    ...
    return this.drawResults
      ...
      .map((item) => {
        ...
        const checkin = this.checkins.find(
          (checkinItem) => checkinItem.anonymous_id === item.anonymous_id && checkinItem.mode === item.mode
        );
        return {
          id: item.id,
          ...
          venue_id: checkin?.venue_id ?? null
        };
      })
...
async function simulateBrowserCheckin(db, storage, requestId, body = {}) {
  ...
  if (storedAnonymousId) {
    return {
      submitted: false,
      result: await getResult(db, storedAnonymousId, mode)
    };
  }
...
async function testResultRedisplay() {
  ...
  const firstReload = await getResult(db, original.anonymous_id, "live");
  const secondReload = await getResult(db, original.anonymous_id, "live");
...
async function testClaimedResultRedisplay() {
  ...
  const redisplayed = await getResult(db, winner.anonymous_id, "live");
...
async function testPrizeTotalChanges() {
  ...
  assert.equal((await getResult(db, first.anonymous_id, "live")).result, "win", "既存当選結果は変わらない");
